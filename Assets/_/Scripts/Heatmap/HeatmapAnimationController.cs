...
public class HeatmapAnimationController : MonoBehaviour
{
    ...
    [Header("顏色映射")]
    [Range(1f, 4f)] public float rampBias = 1.5f;
    [Range(0.5f, 3f)] public float rampContrast = 1.0f;

    static readonly int ID_RampBias = Shader.PropertyToID("_RampBias");
    static readonly int ID_RampContrast = Shader.PropertyToID("_RampContrast");
    ...
    void Update()
    {
        ...
        // Update() 裡加入
        _mpb.SetFloat(ID_RampBias, rampBias);
        _mpb.SetFloat(ID_RampContrast, rampContrast);
